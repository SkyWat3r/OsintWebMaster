...
Stroke drawings use a `network-patch` matcher that compares the whole traced shape against normalized local OSM road-network patches with optional rotation. Smoothed curves are sampled into denser strokes before matching. Older point/edge drawings still fall back to `linear-trace` or intersection matching. Points with two connected segments are treated as bends. Points with three or more connected segments are treated as intersections. `Free rotation` keeps the search independent from absolute north/south orientation.
